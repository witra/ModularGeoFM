...
class SatMAEHeadViT(nn.Module):
    ...
    Included here under the terms of the Apache 2.0 license.
    ...
    def __init__(self, embed_dim: int = None, out_channels: int = None, bias: bool = True,
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.num_patches + 1, self.out_channels),
                                              requires_grad=False)  # fixed sin-cos embedding

        self.decoder_blocks = nn.ModuleList([
            Block(self.out_channels, self.num_heads, self.mlp_ratio, qkv_bias=True, qk_norm=None,
                  norm_layer=self.norm_layer)
            for i in range(self.depth)])

        self.decoder_norm = self.norm_layer(self.out_channels)
        self.decoder_pred = nn.Linear(self.out_channels, self.patch_size ** 2 * self.in_chans,
