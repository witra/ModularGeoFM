...
class CopernicusFMViT(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed_spectral = Dynamic_MLP_OFA_spectral(wv_planes=128, inter_dim=128, kernel_size=16,
                                                             embed_dim=embed_dim)
        self.patch_embed_variable = Dynamic_MLP_OFA_variable(wv_planes=128, inter_dim=128, kernel_size=16,
                                                             embed_dim=embed_dim)
        ...
        self.time_expansion = FourierExpansion(1, 365.25,
                                               assert_range=False)  # 1 to 365.25 days, enable more than 1 year
    ...
    def get_coord_pos_embed(self, lons, lats, embed_dim):
        if self.loc_option == 'cartesian':
            ...
            coord_embed = torch.cat([coord_embed_spherical_x, coord_embed_spherical_y, coord_embed_spherical_z],
                                    dim=-1)  # [B,D]
        ...
        if coord_embed.shape[-1] < embed_dim:
            # pad zeros
            coord_embed = torch.cat((coord_embed, torch.zeros(coord_embed.shape[0], embed_dim - coord_embed.shape[-1],
                                                              device=coord_embed.device)), dim=-1)
    ...
    def forward_features(self, x, meta_info, wave_list, bandwidth, language_embed, input_mode, kernel_size=None):
        ...
        pos_embed = resize_abs_pos_embed(self.pos_embed, num_patches_sqrt,
                                         (num_patches_sqrt_origin, num_patches_sqrt_origin), num_prefix_tokens=1)
    ...
    def forward(self, x, meta_info, wave_list, bandwidth, language_embed, input_mode, kernel_size=None):
        if self.return_intermediate:
            x, intermediate_features = self.forward_features(x, meta_info, wave_list, bandwidth, language_embed,
                                                             input_mode, kernel_size)
